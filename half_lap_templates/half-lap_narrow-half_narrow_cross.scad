...
as_mm(convert_to_mm) {
    ...
    difference() {
        ...
        translate([board_narrow_face,-1 * template_wall_thickness, board_wide_face * 0.5]){
            cube([
              board_narrow_face/2,
          ...
          board_wide_face + template_wall_thickness
            ...
            rotate([0,0,-90]){
                #linear_extrude(2 * template_wall_thickness){
                    text(str("1/2 ", nominal_size), size=0.25, font="Liberation Mono:style=Bold", halign="center", valign="bottom");
                }
            }
        }
        // URL text doesn't fit
    }
}
